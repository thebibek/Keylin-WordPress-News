<?php
if (!class_exists('ATBS_Module_Post_No_Thumb_1')) {
    class ATBS_Module_Post_No_Thumb_1 {
        ...
        function render($postAttr) {
            ...
            <article class="post post--no-thumb <?php if ( !(ATBS_Core::bk_check_has_post_thumbnail($postID) && isset($postAttr['thumbSize']) && ($postAttr['thumbSize'] != ''))): echo 'post-not-exist-thumb'; endif;?> <?php if (isset($postAttr['additionalClass']) && ($postAttr['additionalClass'] != null)) echo esc_attr($postAttr['additionalClass']);?>" data-dark-mode="<?php echo esc_attr($data_dark_mode);?>">
                <?php
                if (isset($postAttr['thumb']) && ($postAttr['thumb'] != '')):
                    if ( !(ATBS_Core::bk_check_has_post_thumbnail($postID) && isset($postAttr['thumbSize']) && ($postAttr['thumbSize'] != ''))): ?>
                    <div class="post__thum <?php if (isset($postAttr['additionalThumbClass']) && ($postAttr['additionalThumbClass'] != null)) echo esc_attr($postAttr['additionalThumbClass']);?>b"><span class="atbs-pseudo-img"></span></div>
                <?php else: ?>
