...
<?php
    ...
    $featuredMediaExists = 1;
    if( ATBS_Core::bk_check_has_post_thumbnail($postID) == 0 ){
        $featuredMediaExists = 0;
    }
...
?>
<div class="site-content single-entry single-style-9 <?php echo esc_attr($infinity_wrapDivClass);?>">
    <div class="atbs-keylin-block atbs-keylin-block--fullwidth single-entry-container <?php if($infinityScrolling != 0): echo 'single-infinity-container'; endif;?>">
        <div class="atbs-keylin-block single-entry-section-wrap single-style-9 single-9 atbs-entry-title-fw <?php if ($bkHeaderInverse == 1) {echo 'single-entry--template-4';} else {echo 'single-entry--template-4-alt';}?> <?php if($featuredMediaExists == 0) echo ' featured-media-not-exists'; ?> atbs-keylin-block-<?php echo esc_attr($currentPostID);?> element-scroll-percent<?php echo esc_attr($scrollSinglePercentageProgressClass);?> <?php if($infinityScrolling != 0): echo 'single-infinity-inner'; endif;?>" data-url-to-load="<?php echo esc_url($infinity_PermalinkToLoad);?>" data-postid="<?php echo esc_attr($currentPostID);?>">
            <div class="atbs-keylin-block atbs-keylin-block--fullwidth atbs-keylin-block--contiguous single-header-with-bg <?php if ($bkHeaderBGPattern != 0) echo 'single-header--has-pattern';?>" <?php echo esc_attr($styleBGColor);?>>
                <div class="container<?php echo esc_attr($classContainerNarrow); ?>">
                    <header class="single-header<?php if ($bkHeaderInverse == 1): echo ' inverse-text'; else: echo ' text-black'; endif; ?>">
                                    ...
                                    <?php
                                        ...
                                        if (($postFormat == 'video') || ($postFormat == 'gallery')) {
                                            ...
                                            if (($featuredImageSTS != 0) && (ATBS_Core::bk_check_has_post_thumbnail($postID))) {
                                                echo '<div class="entry-thumb single-entry-thumb img-object-fit">';
                                                echo get_the_post_thumbnail($postID, 'atbs-m-2_1');
                                                if ($featImgCaption != '') {
