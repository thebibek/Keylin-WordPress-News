...
<?php
    ...
    $hasThumbnail       = ATBS_Core::bk_check_has_post_thumbnail($postID);
    ...
    $infinityScrolling  = isset($atbs_option['single-sections-infinity-scrolling']) ? $atbs_option['single-sections-infinity-scrolling'] : 0;
...
<div class="site-content single-entry <?php echo esc_attr($infinity_wrapDivClass);?>">
    <div class="atbs-keylin-block atbs-keylin-block--fullwidth single-entry-wrap <?php if ($infinityScrolling != 0): echo 'single-infinity-container'; endif;?>">
        <div class="atbs-keylin-block single-entry-section-wrap single-style-3 single-entry--no-sidebar single-3<?php if ($featuredImageSTS && $hasThumbnail) echo ' atbs-entry-title-fw'; ?> atbs-keylin-block-<?php echo esc_attr($currentPostID);?> atbs-keylin-block--fullwidth element-scroll-percent<?php echo esc_attr($scrollSinglePercentageProgressClass);?> <?php if ($infinityScrolling != 0): echo 'single-infinity-inner'; endif;?>" data-url-to-load="<?php echo esc_url($infinity_PermalinkToLoad);?>" data-postid="<?php echo esc_attr($currentPostID);?>">
                <article <?php post_class('atbs-keylin-block post--single');?>>
                    <div class="single-content">
                        <header class="single-header">
                            <div class="container<?php if (!$featuredImageSTS || !$hasThumbnail) echo ' container--narrow'; ?>">
                            <?php
                        ...
                        if (($postFormat == 'video') || ($postFormat == 'gallery')) {
                            if (ATBS_Single::bk_entry_media($postID) != '') {
                            ...
                            if ($featuredImageSTS && $hasThumbnail) {
                                echo '<div class="entry-thumb single-entry-thumb img-object-fit single-thumb-max-height">';
                                echo get_the_post_thumbnail($postID, 'atbs-xxl');
                                if ($featImgCaption != '') :
                ...
                <?php
                    ...
                    if (($bkPostNavSW != '') && ($bkPostNavSW != 0)) {
                        echo ATBS_Single::bk_post_navigation('wide');
                    }
                ?>
                </div><!-- .container -->
